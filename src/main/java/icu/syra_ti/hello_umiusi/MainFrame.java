...
public class MainFrame extends JFrame {
    ...
    private MainFrame(String startURL, boolean useOSR, boolean isTransparent, String[] args) throws UnsupportedPlatformException, CefInitializationException, IOException, InterruptedException {
        ...
        UmiusiInstaller umiusiInstaller = new UmiusiInstaller();
        // (0) Initialize CEF using the maven loader
        CefAppBuilder builder = new CefAppBuilder();
        File customBundleDir = new File(UmiusiUtil.getResourceDir() + "/jcef-bundle_" + System.getProperty("os.arch"));
        builder.setInstallDir(customBundleDir);
        // windowless_rendering_enabled must be set to false if not wanted. 
        builder.getCefSettings().windowless_rendering_enabled = useOSR;
        builder.getCefSettings().remote_debugging_port = 9222;
        ...
        CefMessageRouter msgRouter = CefMessageRouter.create(new CefMessageRouter.CefMessageRouterConfig("umiusiBridge", "umiusiBridgeCancel"));
        msgRouter.addHandler(new CefMessageRouterHandlerAdapter() {
            @Override
            public boolean onQuery(CefBrowser cefBrowser, CefFrame cefFrame, long query_id, String request, boolean persistent, CefQueryCallback cefQueryCallback) {
                if (request.equals("GET_LATEST")) {
                    executor.submit(() -> {
                        browser_.executeJavaScript(
                                "window.umiusiBridgeMessageHandler.lockGit()"
                                , startURL, 0);
                        umiusiInstaller.repoPull(new ProgressMonitor() {
                            int totalWork = 0;
                            int finishedWork = 0;

                            public void printToPage(String message) {
                                browser_.executeJavaScript(
                                        String.format("window.umiusiBridgeMessageHandler.log('%s')", message)
                                        , startURL, 0);
                            }

                            @Override
                            public void start(int totalTasks) {
                                this.printToPage("Pull开始...");
                            }
                            ...
                            public void beginTask(String title, int totalWork) {
                                ...
                                this.finishedWork = 0;
                                this.printToPage("任务开始: " + title + ", 总任务数: " + totalWork);
                            }

                            @Override
                            public void update(int completed) {
                                this.finishedWork += completed;
                                this.printToPage("进度: " + this.finishedWork + "/" + this.totalWork);

                            }

                            @Override
                            public void endTask() {
                                this.printToPage("任务结束");
                            }
                        ...
                        browser_.executeJavaScript(
                                "window.umiusiBridgeMessageHandler.unlockGit()"
                                , startURL, 0);
                    });
                }
                return true;
...
//        getContentPane().add(address_, BorderLayout.NORTH);
    ...
    public static void main(String[] args) throws UnsupportedPlatformException, CefInitializationException, IOException, InterruptedException {
        ...
        String url = "file://" + UmiusiUtil.getResourceDir() + "/web/index.html";  // 加载本地 HTML
